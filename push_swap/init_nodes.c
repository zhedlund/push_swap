...
/*   Updated: 2023/09/14 14:05:01 by zhedlund         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*  every node in b gets a target node in a. best match is the next bigger
*	number. If no node is bigger, best match is the smallest.
...
void	set_target_node(t_stack *stack_a, t_stack *stack_b)
{
	...
	while (stack_b)
	{
		...
		while (node_a)
		{
			if ((node_a->number > stack_b->number)
				&& (node_a->number < best_match_index))
			{
...
void	set_cost_to_push(t_stack *stack_a, t_stack *stack_b)
{
	...
	while (stack_b)
	{
		...
		if (stack_b->target_node->top_half)
			...
			stack_b->cost_to_push
				+= a_size - (stack_b->target_node->current_position);
		stack_b = stack_b->next;
	}
}
